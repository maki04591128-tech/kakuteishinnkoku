...
import { simulateAgriculturalLandRationalizationSaleDeduction } from "@/lib/realEstate/agriculturalLandRationalizationSaleDeduction";
...
export function AgriculturalLandRationalizationSaleDeductionForm() {
  ...
  const result = useMemo(() => {
    try {
      return simulateAgriculturalLandRationalizationSaleDeduction({
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Field label="譲渡価額(買取り代金・清算金等)" value={transferPriceJpy} onChange={setTransferPriceJpy} />
      ...
      <div className="flex flex-col gap-2">
        <label className="flex items-center gap-2 text-sm">
          ...
          />
          農地保有の合理化等のための800万円特別控除(措置法34条の3)の要件(あっせん・農地
          中間管理機構への譲渡・清算金の取得等の対象となる譲渡類型に該当すること、確定
          申告書への記載及び証明書類の添付等)を満たす
        </label>
        <label className="flex items-center gap-2 text-sm">
          <input
            type="checkbox"
      ...
      {result === null ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <SummaryCard label="800万円特別控除の適用額" value={result.specialDeductionAppliedJpy} />
