"use client";

import { useMemo, useState } from "react";
import {
  simulateBondInterestIncomeTaxation,
  type InterestIncomeTaxMethod,
  type InterestIncomeTaxMethodResult,
} from "@/lib/investment/interestIncomeTaxSimulation";

const METHOD_LABEL: Record<InterestIncomeTaxMethod, string> = {
  SEPARATE: "申告分離課税",
  NO_FILING: "申告不要",
};

function yen(value: { toString(): string }): string {
  const n = Number(value.toString());
  return `¥${Math.round(n).toLocaleString("ja-JP")}`;
}

export function InterestIncomeSimulatorForm({
  defaultInterestIncomeJpy,
  defaultAvailableListedStockLossJpy,
}: {
  defaultInterestIncomeJpy: number;
  defaultAvailableListedStockLossJpy: number;
}) {
  const [interestIncomeJpy, setInterestIncomeJpy] = useState(
    String(defaultInterestIncomeJpy),
  );
  const [lossJpy, setLossJpy] = useState(String(defaultAvailableListedStockLossJpy));

  const result = useMemo(() => {
    try {
      return simulateBondInterestIncomeTaxation({
        interestIncomeJpy: interestIncomeJpy === "" ? 0 : interestIncomeJpy,
        availableListedStockLossJpy: lossJpy === "" ? 0 : lossJpy,
      });
    } catch {
      return null;
    }
  }, [interestIncomeJpy, lossJpy]);

  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Field
          label="特定公社債の利子等の収入金額(源泉徴収前・年間合計)"
          value={interestIncomeJpy}
          onChange={setInterestIncomeJpy}
        />
        <Field
          label="損益通算できる上場株式等の譲渡損失(当年+繰越)"
          value={lossJpy}
          onChange={setLossJpy}
        />
      </div>

      {result === null ? (
        <p className="text-sm text-red-600">入力値を確認してください(0以上の数値を入力)。</p>
      ) : (
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <MethodCard
              result={result.separate}
              recommended={result.recommendedMethod === "SEPARATE"}
              extra={
                result.separate.lossOffsetUsedJpy
                  ? `損益通算に使用: ${yen(result.separate.lossOffsetUsedJpy)}`
                  : undefined
              }
            />
            <MethodCard
              result={result.noFiling}
              recommended={result.recommendedMethod === "NO_FILING"}
            />
          </div>

          {result.remainingListedStockLossJpy.greaterThan(0) && (
            <p className="text-sm text-neutral-500">
              申告分離課税を選んだ場合の繰越譲渡損失の残額: {yen(result.remainingListedStockLossJpy)}
            </p>
          )}

          <ul className="list-disc space-y-1 pl-5 text-xs text-neutral-500">
            {result.notes.map((note, i) => (
              <li key={i}>{note}</li>
            ))}
          </ul>
        </>
      )}
    </div>
  );
}

function Field({
  label,
  value,
  onChange,
}: {
  label: string;
  value: string;
  onChange: (value: string) => void;
}) {
  return (
    <label className="flex flex-col gap-1 text-sm">
      <span className="text-neutral-500">{label}</span>
      <input
        type="number"
        inputMode="numeric"
        min={0}
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="rounded-md border border-neutral-300 bg-white px-3 py-1.5 dark:border-neutral-700 dark:bg-neutral-900"
      />
    </label>
  );
}

function MethodCard({
  result,
  recommended,
  extra,
}: {
  result: InterestIncomeTaxMethodResult;
  recommended: boolean;
  extra?: string;
}) {
  return (
    <div
      className={`rounded-lg border p-4 ${
        recommended
          ? "border-neutral-900 dark:border-white"
          : "border-neutral-200 dark:border-neutral-800"
      }`}
    >
      <p className="flex items-center justify-between text-sm text-neutral-500">
        <span>{METHOD_LABEL[result.method]}</span>
        {recommended && (
          <span className="rounded-full bg-neutral-900 px-2 py-0.5 text-xs text-white dark:bg-white dark:text-neutral-900">
            有利
          </span>
        )}
      </p>
      <p className="mt-1 text-2xl font-semibold">{yen(result.totalTaxJpy)}</p>
      <p className="mt-1 text-xs text-neutral-400">
        国税 {yen(result.nationalTaxJpy)} + 住民税 {yen(result.residentTaxJpy)}
      </p>
      {extra && <p className="mt-1 text-xs text-neutral-400">{extra}</p>}
    </div>
  );
}
