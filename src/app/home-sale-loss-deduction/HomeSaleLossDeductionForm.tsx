...
} from "@/lib/realEstate/homeSaleLossCarryforward";
...
export function HomeSaleLossDeductionForm({
  year,
  registeredDeduction,
  carryforwardEntries,
}: {
  ...
  const result = useMemo(() => {
    try {
      return calculateHomeSaleLoss({
        ...
        mortgageBalanceJpy: mortgageBalance === "" ? 0 : mortgageBalance,
  ...
  const carryforwardResult = useMemo(() => {
    ...
    try {
      return calculateHomeSaleLossCarryforward(
        year,
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 gap-4">
        <Field label="譲渡価額(売却代金)" value={transferPrice} onChange={setTransferPrice} />
        <Field label="取得費" value={acquisitionCost} onChange={setAcquisitionCost} />
        <Field
        ...
        />
        <Field
          label="譲渡した年の1月1日時点の所有期間(年)"
          value={ownershipYears}
          onChange={setOwnershipYears}
        />
        <Field
          label="譲渡契約締結日の前日時点の住宅借入金等残高(償還期間10年以上のもの)"
      ...
      {result === null ? (
        <p className="text-sm text-red-600">
          入力値を確認してください(金額は0以上、所有期間は0以上の整数)。
        </p>
      ) : (
        <>
          ...
          <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
            ...
            />
            <DetailItem
              label="住宅借入金等残高の要件(残高が譲渡価額を上回る)"
              value={result.mortgageConditionEligible ? "満たす" : "満たさない"}
            />
            <DetailItem
              label="損益通算限度額(住宅借入金等残高-譲渡価額)"
              value={yen(result.offsetLimitJpy)}
            />
          ...
          <div className="rounded-md border border-neutral-200 p-4 dark:border-neutral-800">
            <form action={saveIncomeDeduction}>
              ...
              <input type="hidden" name="type" value="HOME_SALE_LOSS" />
              <input
                type="hidden"
              ...
              />
              <input type="hidden" name="redirectPath" value="/home-sale-loss-deduction" />
              <button
                type="submit"
          ...
          {carryforwardResult !== null && carryforwardResult.newLossJpy.greaterThan(0) && (
            <div className="rounded-md border border-neutral-200 p-4 dark:border-neutral-800">
              ...
              <form action={carryForwardHomeSaleLossExcess} className="mt-3">
                <input type="hidden" name="year" value={year} />
                <input
                  type="hidden"
