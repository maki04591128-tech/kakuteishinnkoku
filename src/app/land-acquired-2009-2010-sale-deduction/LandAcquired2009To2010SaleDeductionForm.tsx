...
import { simulateLandAcquired2009To2010SaleDeduction } from "@/lib/realEstate/landAcquired2009To2010SaleDeduction";
...
export function LandAcquired2009To2010SaleDeductionForm() {
  ...
  const result = useMemo(() => {
    try {
      return simulateLandAcquired2009To2010SaleDeduction({
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2">
        <label className="flex items-center gap-2 text-sm">
          ...
          から平成22年12月31日までの間に取得したこと、特別な関係がある者からの取得・
          相続や贈与等による取得でないこと等)を満たす
        </label>
        <label className="flex items-center gap-2 text-sm">
          <input
            type="checkbox"
      ...
      {result === null ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <SummaryCard label="1,000万円特別控除の適用額" value={result.specialDeductionAppliedJpy} />
          ...
          <div className="rounded-lg border border-neutral-900 p-4 dark:border-white">
            <p className="text-sm text-neutral-500">
              {result.holdingPeriodCategory === "LONG_TERM"
                ? "長期譲渡所得(税率合計20.315%)"
                : "短期譲渡所得(税率合計39.63%。特別控除は長期譲渡所得のみ対象のため未適用)"}
            </p>
