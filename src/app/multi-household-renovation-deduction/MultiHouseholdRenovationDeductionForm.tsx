...
} from "@/lib/multiHouseholdRenovationDeduction";
...
const FLOOR_AREA_CATEGORY_LABEL: Record<MultiHouseholdRenovationFloorAreaCategory, string> = {
  AT_LEAST_50: "50平方メートル以上",
  FROM_40_TO_50: "40平方メートル以上50平方メートル未満(令和8年以後の居住のみ)",
};

export function MultiHouseholdRenovationDeductionForm({
  year,
  registeredCreditJpy,
}: {
  year: number;
  ...
  const [floorAreaCategory, setFloorAreaCategory] =
    useState<MultiHouseholdRenovationFloorAreaCategory>("AT_LEAST_50");
  const [totalIncomeJpy, setTotalIncomeJpy] = useState("0");
  const [duplicatedKitchen, setDuplicatedKitchen] = useState(true);
  const [duplicatedBathroom, setDuplicatedBathroom] = useState(true);
  const [duplicatedToilet, setDuplicatedToilet] = useState(false);
  const [duplicatedEntrance, setDuplicatedEntrance] = useState(false);
  ...
  const result = useMemo(() => {
    try {
      return estimateMultiHouseholdRenovationDeduction({
    ...
    <div className="flex flex-col gap-6">
      <fieldset className="grid grid-cols-1 gap-4 rounded-md border border-neutral-200 p-3 sm:grid-cols-2 dark:border-neutral-800">
        ...
        <label className="flex flex-col gap-1 text-sm">
          ...
          <select
            ...
            onChange={(e) =>
              setFloorAreaCategory(e.target.value as MultiHouseholdRenovationFloorAreaCategory)
            }
          ...
          >
            {(
              ...
              ) as MultiHouseholdRenovationFloorAreaCategory[]
            ).map((c) => (
      ...
      <fieldset className="flex flex-wrap gap-4 rounded-md border border-neutral-200 p-3 text-sm dark:border-neutral-800">
        <legend className="px-1 text-sm font-medium">
          改修工事により複数になった室(いずれか2以上が要件)
        </legend>
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            checked={duplicatedKitchen}
            onChange={(e) => setDuplicatedKitchen(e.target.checked)}
          />
          調理室
        </label>
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            checked={duplicatedBathroom}
            onChange={(e) => setDuplicatedBathroom(e.target.checked)}
          />
          浴室
        </label>
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            checked={duplicatedToilet}
            onChange={(e) => setDuplicatedToilet(e.target.checked)}
          />
          便所
        </label>
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            checked={duplicatedEntrance}
            onChange={(e) => setDuplicatedEntrance(e.target.checked)}
          />
          玄関
        </label>
      </fieldset>
        ...
        <>
          <div className="rounded-lg border border-neutral-900 p-6 dark:border-white">
            <p className="text-sm text-neutral-500">
              多世帯同居改修工事の住宅特定改修特別税額控除額(所得税分のみ)
            </p>
          ...
          <div className="rounded-md border border-neutral-200 p-4 dark:border-neutral-800">
            <form action={saveMultiHouseholdRenovationDeductionRecord}>
              ...
              >
                この試算結果を{year}
                年分の多世帯同居改修工事に係る住宅特定改修特別税額控除として登録する
              </button>
            </form>
            {registeredCreditJpy !== null && (
              <div className="mt-2 flex flex-wrap items-center gap-3">
                ...
                <form action={deleteMultiHouseholdRenovationDeductionRecord}>
                  <input type="hidden" name="year" value={year} />
                  <button
                    type="submit"
