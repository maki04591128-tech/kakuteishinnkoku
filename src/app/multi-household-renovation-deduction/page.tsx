...
export default async function MultiHouseholdRenovationDeductionPage({
  searchParams,
}: {
    ...
    <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-8 p-6 sm:p-10">
      <header>
        ...
        <h1 className="mt-2 text-2xl font-bold tracking-tight">
          多世帯同居改修工事の住宅特定改修特別税額控除の試算({year}年分)
        </h1>
        <p className="mt-1 text-sm text-neutral-500">
          自己が所有する居住用家屋について複数世帯が同居しやすくするための多世帯同居
          改修工事を行った場合、
          <Link href={`/barrier-free-renovation-deduction?year=${year}`} className="underline">
            バリアフリー改修工事
          </Link>
          や
          ...
          の住宅特定改修特別税額控除と同様に借入金の有無を問わず、その年分の所得税額から
          控除できる(租税特別措置法41条の19の3)。単年で完結する控除で繰越制度は無い。
        </p>
      </header>
      ...
      {params.saved !== undefined && (
        <p className="rounded-md bg-green-50 px-4 py-2 text-sm text-green-700 dark:bg-green-950 dark:text-green-300">
          {year}年分の多世帯同居改修工事の住宅特定改修特別税額控除を登録しました。下書きCSV
      ...
      {params.deleted !== undefined && (
        <p className="rounded-md bg-neutral-50 px-4 py-2 text-sm text-neutral-600 dark:bg-neutral-900 dark:text-neutral-300">
          {year}年分の多世帯同居改修工事の住宅特定改修特別税額控除の登録を削除しました。
        </p>
      )}

      <MultiHouseholdRenovationDeductionForm
        year={year}
      ...
      <div className="flex flex-col gap-2 rounded-md border border-dashed border-neutral-300 p-4 text-xs text-neutral-500 dark:border-neutral-700">
        <p>
          「多世帯同居改修工事に係る標準的な費用の額」は、工事の内容に応じて政令で
        ...
        <p>
          対象となる多世帯同居改修工事は、調理室・浴室・便所・玄関のうちいずれか2以上
          について、改修後に自己の居住の用に供する部分でそれぞれ複数になる工事に限られる
          (国税庁タックスアンサーNo.1224)。
        </p>
        <p>
          住民税に相当する控除制度は存在しないため、所得税額からのみ控除する
          (バリアフリー改修工事・省エネ改修工事の住宅特定改修特別税額控除と同様)。
          この控除はバリアフリー改修・省エネ改修・住宅耐震改修特別控除・耐久性向上改修・
          子育て対応改修の各制度といずれか1つの選択適用となり、選択替えはできない。
          これらとの選択適用・併用時の限度額の合算判定は別制度・別計算のため本ツールでは
          試算しない(今後の課題)。
        </p>
        <p>
