...
} from "@/lib/energySavingRenovationDeduction";
...
const FLOOR_AREA_CATEGORY_LABEL: Record<EnergySavingRenovationFloorAreaCategory, string> = {
  AT_LEAST_50: "50平方メートル以上",
  FROM_40_TO_50: "40平方メートル以上50平方メートル未満(令和8年以後の居住のみ)",
};

export function EnergySavingRenovationDeductionForm({
  year,
  registeredCreditJpy,
}: {
  year: number;
  ...
  const [floorAreaCategory, setFloorAreaCategory] =
    useState<EnergySavingRenovationFloorAreaCategory>("AT_LEAST_50");
  const [totalIncomeJpy, setTotalIncomeJpy] = useState("0");
  const [standardCostJpy, setStandardCostJpy] = useState("0");
  const [includesSolarPanelWork, setIncludesSolarPanelWork] = useState(false);
    ...
    <div className="flex flex-col gap-6">
      <fieldset className="grid grid-cols-1 gap-4 rounded-md border border-neutral-200 p-3 sm:grid-cols-2 dark:border-neutral-800">
        ...
        <label className="flex flex-col gap-1 text-sm">
          ...
          <select
            ...
            onChange={(e) =>
              setFloorAreaCategory(e.target.value as EnergySavingRenovationFloorAreaCategory)
            }
            className="rounded-md border border-neutral-300 bg-white px-3 py-1.5 dark:border-neutral-700 dark:bg-neutral-900"
          >
            {(
              Object.keys(FLOOR_AREA_CATEGORY_LABEL) as EnergySavingRenovationFloorAreaCategory[]
            ).map((c) => (
      ...
      <fieldset className="flex flex-wrap gap-4 rounded-md border border-neutral-200 p-3 text-sm dark:border-neutral-800">
        ...
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            checked={includesSolarPanelWork}
            onChange={(e) => setIncludesSolarPanelWork(e.target.checked)}
          />
          太陽光発電設備設置工事を含む(控除対象限度額が250万円→350万円)
        </label>
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
      ...
      {result === null ? (
        ...
        <>
          ...
          <div className="rounded-md border border-neutral-200 p-4 dark:border-neutral-800">
            <form action={saveEnergySavingRenovationDeductionRecord}>
              ...
              >
                この試算結果を{year}
                年分の省エネ改修工事に係る住宅特定改修特別税額控除として登録する
              </button>
            </form>
            {registeredCreditJpy !== null && (
              <div className="mt-2 flex flex-wrap items-center gap-3">
                ...
                <form action={deleteEnergySavingRenovationDeductionRecord}>
                  <input type="hidden" name="year" value={year} />
                  <button
                    type="submit"
