  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-8 p-6 sm:p-10">
      ...
      <p className="rounded-md border border-dashed border-neutral-300 p-4 text-xs text-neutral-500 dark:border-neutral-700">
        ...
        扶養親族・同一生計配偶者の合計所得金額要件(48万円→58万円)の変更のみ年分
        ({year}年分)に応じて自動的に切り替える(令和7年分・2025年分以後は58万円、
        令和6年分・2024年分以前は48万円)。配偶者特別控除は上限(133万円)・控除額の
        段階表自体に変更が無いため、この要件緩和の反映のみで対応できている。
        一方、基礎控除の引き上げ(所得金額に応じた段階表への変更)と、19〜22歳の親族
        向けに新設された「特定親族特別控除」は、控除額の段階表を一次情報で確実に
        確認できておらず未対応(該当する場合は国税庁「確定申告書等作成コーナー」の
        最新の控除額で別途計算すること)。「この試算結果を{year}年分の所得控除として
        登録する」ボタンで登録すると、`/tax-estimate`の「給与所得等の課税所得金額」の
        初期値にこの控除額が自動反映される(登録後も入力欄は手入力で上書き可能)。
      </p>
    </div>
  );
}
