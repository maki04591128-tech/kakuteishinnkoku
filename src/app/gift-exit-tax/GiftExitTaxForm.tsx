...
import { estimateGiftExitTax } from "@/lib/investment/giftExitTax";
...
export function GiftExitTaxForm() {
  ...
  const result = useMemo(() => {
    try {
      return estimateGiftExitTax({
        holdings: holdings.map((row) => ({
          symbol: row.symbol,
          isListed: row.isListed,
          isGiftedAsset: row.isGiftedAsset,
      ...
      <div className="flex flex-col gap-3">
        {holdings.map((row, index) => (
          ...
          >
            ...
            <label className="flex flex-col gap-1 text-sm">
              <span className="text-neutral-500">贈与した部分か</span>
              <select
                value={row.isGiftedAsset ? "gifted" : "kept"}
                onChange={(e) =>
                  setHoldings((prev) =>
                    prev.map((r) =>
                      r.key === row.key
                        ? { ...r, isGiftedAsset: e.target.value === "gifted" }
                        : r,
                    ),
                  )
                }
                className="rounded-md border border-neutral-300 bg-white px-3 py-1.5 dark:border-neutral-700 dark:bg-neutral-900"
              >
                <option value="gifted">贈与した(課税対象)</option>
                <option value="kept">贈与していない(1億円判定のみ)</option>
              </select>
            </label>
            <label className="flex flex-col gap-1 text-sm">
            ...
            <label className="flex flex-col gap-1 text-sm">
              <span className="text-neutral-500">贈与時点の時価(単価)</span>
              <input
                type="number"
                inputMode="numeric"
                min={0}
        ...
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-lg border border-neutral-200 p-4 dark:border-neutral-800">
              <p className="text-sm text-neutral-500">保有する対象資産の価額の合計</p>
              <p className="mt-1 text-2xl font-semibold">
                {yen(result.totalHoldingsMarketValueJpy)}
              </p>
            ...
            <div className="rounded-lg border border-neutral-200 p-4 dark:border-neutral-800">
              <p className="text-sm text-neutral-500">国外転出(贈与)時課税の対象</p>
              <p className="mt-1 text-2xl font-semibold">
                {result.isSubjectToGiftExitTax ? "対象" : "対象外"}
              </p>
              <p className="mt-1 text-xs text-neutral-500">
                居住期間要件(5年超):{" "}
            ...
            <div className="rounded-lg border border-neutral-200 p-4 dark:border-neutral-800">
              <p className="text-sm text-neutral-500">課税対象額の合計(贈与した部分のみ)</p>
