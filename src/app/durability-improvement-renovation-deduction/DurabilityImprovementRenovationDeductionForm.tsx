...
import {
  ...
  type DurabilityImprovementRenovationFloorAreaCategory,
} from "@/lib/durabilityImprovementRenovationDeduction";
...
const FLOOR_AREA_CATEGORY_LABEL: Record<DurabilityImprovementRenovationFloorAreaCategory, string> =
  {
    AT_LEAST_50: "50平方メートル以上",
    FROM_40_TO_50: "40平方メートル以上50平方メートル未満(令和8年以後の居住のみ)",
  };

const COMBINATION_TYPE_LABEL: Record<DurabilityImprovementRenovationCombinationType, string> = {
  EARTHQUAKE: "住宅耐震改修と併せて行う",
  ENERGY_SAVING: "一般省エネ改修工事と併せて行う",
  BOTH: "住宅耐震改修・一般省エネ改修工事の両方と併せて行う",
};

export function DurabilityImprovementRenovationDeductionForm({
  year,
  registeredCreditJpy,
}: {
  year: number;
  ...
  const [combinationType, setCombinationType] =
    useState<DurabilityImprovementRenovationCombinationType>("EARTHQUAKE");
  const [floorAreaCategory, setFloorAreaCategory] =
    useState<DurabilityImprovementRenovationFloorAreaCategory>("AT_LEAST_50");
  const [totalIncomeJpy, setTotalIncomeJpy] = useState("0");
  const [includesSolarPowerEquipment, setIncludesSolarPowerEquipment] = useState(false);
  const [earthquakeRenovationStandardCostJpy, setEarthquakeRenovationStandardCostJpy] =
    useState("0");
  const [energySavingRenovationStandardCostJpy, setEnergySavingRenovationStandardCostJpy] =
    useState("0");
  const [durabilityImprovementStandardCostJpy, setDurabilityImprovementStandardCostJpy] =
    useState("0");
  ...
  const [claimedEnergySavingAloneInPastThreeYears, setClaimedEnergySavingAloneInPastThreeYears] =
    useState(false);

  const usesEarthquake = combinationType === "EARTHQUAKE" || combinationType === "BOTH";
  const usesEnergySaving = combinationType === "ENERGY_SAVING" || combinationType === "BOTH";

  const result = useMemo(() => {
    try {
      return estimateDurabilityImprovementRenovationDeduction({
        residenceYear: Number(residenceYear) || 0,
        combinationType,
        floorAreaCategory,
        totalIncomeJpy: totalIncomeJpy || 0,
        includesSolarPowerEquipment,
        earthquakeRenovationStandardCostJpy: earthquakeRenovationStandardCostJpy || 0,
        energySavingRenovationStandardCostJpy: energySavingRenovationStandardCostJpy || 0,
        durabilityImprovementStandardCostJpy: durabilityImprovementStandardCostJpy || 0,
        ...
        claimedEnergySavingAloneInPastThreeYears,
      });
    } catch {
      return null;
    }
    ...
    totalIncomeJpy,
    includesSolarPowerEquipment,
    earthquakeRenovationStandardCostJpy,
    ...
    <div className="flex flex-col gap-6">
      <fieldset className="grid grid-cols-1 gap-4 rounded-md border border-neutral-200 p-3 sm:grid-cols-2 dark:border-neutral-800">
        ...
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-neutral-500">併せて行う工事</span>
          <select
            value={combinationType}
            onChange={(e) =>
              setCombinationType(e.target.value as DurabilityImprovementRenovationCombinationType)
            }
            className="rounded-md border border-neutral-300 bg-white px-3 py-1.5 dark:border-neutral-700 dark:bg-neutral-900"
          >
            {(
              Object.keys(COMBINATION_TYPE_LABEL) as DurabilityImprovementRenovationCombinationType[]
            ).map((c) => (
              <option key={c} value={c}>
                {COMBINATION_TYPE_LABEL[c]}
              </option>
            ))}
          </select>
        </label>
        <label className="flex flex-col gap-1 text-sm">
          ...
          <select
            ...
            onChange={(e) =>
              setFloorAreaCategory(
                e.target.value as DurabilityImprovementRenovationFloorAreaCategory,
              )
            }
            className="rounded-md border border-neutral-300 bg-white px-3 py-1.5 dark:border-neutral-700 dark:bg-neutral-900"
          >
            {(
              Object.keys(
                FLOOR_AREA_CATEGORY_LABEL,
              ) as DurabilityImprovementRenovationFloorAreaCategory[]
            ).map((c) => (
              <option key={c} value={c}>
        ...
        {usesEarthquake && (
          <Field
            label="住宅耐震改修工事に係る標準的な費用の額(円)"
            helper="増改築等工事証明書に記載された、補助金等控除後の金額をそのまま入力"
            value={earthquakeRenovationStandardCostJpy}
            onChange={setEarthquakeRenovationStandardCostJpy}
          />
        )}
        {usesEnergySaving && (
          <>
            <Field
              label="一般省エネ改修工事に係る標準的な費用の額(円)"
              helper="増改築等工事証明書に記載された、補助金等控除後の金額をそのまま入力"
              value={energySavingRenovationStandardCostJpy}
              onChange={setEnergySavingRenovationStandardCostJpy}
            />
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={includesSolarPowerEquipment}
                onChange={(e) => setIncludesSolarPowerEquipment(e.target.checked)}
              />
              一般省エネ改修工事に太陽光発電設備設置工事を含む
            </label>
          </>
        )}
        <Field
          label="耐久性向上改修工事に係る標準的な費用の額(円)"
          helper="増改築等工事証明書に記載された、補助金等控除後の金額をそのまま入力"
          value={durabilityImprovementStandardCostJpy}
          onChange={setDurabilityImprovementStandardCostJpy}
        />
        <Field
      ...
      <fieldset className="flex flex-wrap gap-4 rounded-md border border-neutral-200 p-3 text-sm dark:border-neutral-800">
        ...
        <label className="flex items-center gap-2">
          ...
          />
          前年以前3年内に同一住宅でこの控除(耐久性向上改修工事分)の適用を受けたことがある
        </label>
        {usesEnergySaving && (
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={claimedEnergySavingAloneInPastThreeYears}
              onChange={(e) => setClaimedEnergySavingAloneInPastThreeYears(e.target.checked)}
            />
            前年以前3年内に同一住宅で一般省エネ改修工事単独の住宅特定改修特別税額控除の
            適用を受けたことがある
          </label>
        )}
      </fieldset>
        ...
        <>
          <div className="rounded-lg border border-neutral-900 p-6 dark:border-white">
            <p className="text-sm text-neutral-500">
              耐久性向上改修工事の住宅特定改修特別税額控除額(所得税分のみ)
            </p>
            ...
            <p className="mt-1 text-xs text-neutral-400">
              ...
              (控除対象限度額 {yen(result.controlLimitJpy)})
            </p>
          </div>

          <div className="rounded-md border border-neutral-200 p-4 dark:border-neutral-800">
            <form action={saveDurabilityImprovementRenovationDeductionRecord}>
              ...
              >
                この試算結果を{year}
                年分の耐久性向上改修工事に係る住宅特定改修特別税額控除として登録する
              </button>
            </form>
            {registeredCreditJpy !== null && (
              <div className="mt-2 flex flex-wrap items-center gap-3">
                ...
                <form action={deleteDurabilityImprovementRenovationDeductionRecord}>
                  <input type="hidden" name="year" value={year} />
                  <button
                    type="submit"
