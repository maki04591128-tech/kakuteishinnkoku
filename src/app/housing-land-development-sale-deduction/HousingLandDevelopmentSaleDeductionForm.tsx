...
import { simulateHousingLandDevelopmentSaleDeduction } from "@/lib/realEstate/housingLandDevelopmentSaleDeduction";
...
export function HousingLandDevelopmentSaleDeductionForm() {
  ...
  const result = useMemo(() => {
    try {
      return simulateHousingLandDevelopmentSaleDeduction({
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Field label="譲渡価額(買取り代金)" value={transferPriceJpy} onChange={setTransferPriceJpy} />
      ...
      <div className="flex flex-col gap-2">
        <label className="flex items-center gap-2 text-sm">
          ...
          買取りであること等。民間の一団の宅地造成(3号)は令和5年12月31日までの
          譲渡に限られる点も含む)を満たす
        </label>
        <label className="flex items-center gap-2 text-sm">
          <input
      ...
      {result === null ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <SummaryCard label="1,500万円特別控除の適用額" value={result.specialDeductionAppliedJpy} />
