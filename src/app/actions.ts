...
import {
  EXCHANGE_LABELS,
  parseExchangeCryptoCsv,
  type CryptoExchange,
} from "@/lib/crypto/exchangeImport";
...
export async function importCryptoExchangeCsv(formData: FormData): Promise<void> {
  const year = Number(requireString(formData, "year"));
  const exchange = requireString(formData, "exchange") as CryptoExchange;
  const file = formData.get("file");
  if (!(file instanceof File) || file.size === 0) {
    throw new Error("CSVファイルを選択してください");
  }
  if (!(exchange in EXCHANGE_LABELS)) {
    throw new Error("未対応の取引所です");
  }

  const text = await file.text();
  const { rows, skippedRows } = parseExchangeCryptoCsv(exchange, text);
