 ...
 *    (所得割10%固定、均等割は含まない。調整控除は`/resident-tax-adjustment-deduction`で
 *    登録済みの場合のみ適用後の額)を前提とする。
...
export function estimateFurusatoNozeiLimit(
  ...
  const notes: string[] = [
    ...
    "住民税所得割額は本ツールの概算合計税額試算による値(所得割10%固定、均等割は含まない。調整控除は登録済みの場合のみ適用後の額)を前提とする。",
