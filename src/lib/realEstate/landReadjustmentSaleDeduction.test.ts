...
import { simulateLandReadjustmentSaleDeduction } from "./landReadjustmentSaleDeduction";

describe("simulateLandReadjustmentSaleDeduction", () => {
  it("長期譲渡所得で譲渡益が2,000万円を超える場合、2,000万円特別控除後の金額に長期税率(20.315%)を適用する", () => {
    const result = simulateLandReadjustmentSaleDeduction({
  ...
  it("短期譲渡所得(所有期間5年以下)の場合、短期税率(39.63%)を適用する", () => {
    ...
    expect(result.specialDeductionAppliedJpy.toNumber()).toBe(20_000_000);
    expect(result.taxableGainJpy.toNumber()).toBe(55_000_000);
    expect(result.nationalTaxJpy.toNumber()).toBeCloseTo(16_846_500, 0);
    expect(result.residentTaxJpy.toNumber()).toBe(4_950_000);
    expect(result.totalTaxJpy.toNumber()).toBeCloseTo(21_796_500, 0);
  });
  ...
  it("譲渡益が控除限度額(2,000万円)未満なら、譲渡益全額を控除して税額は0円になる", () => {
    const result = simulateLandReadjustmentSaleDeduction({
    ...
    expect(result.specialDeductionAppliedJpy.toNumber()).toBe(14_000_000);
  ...
  it("所有期間が負または整数でない場合はエラーになる", () => {
    expect(() =>
      simulateLandReadjustmentSaleDeduction({
        transferPriceJpy: 50_000_000,
        acquisitionCostJpy: 10_000_000,
        transferExpensesJpy: 2_000_000,
        ownershipYears: -1,
