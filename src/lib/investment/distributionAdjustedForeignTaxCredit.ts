 ...
 *
 * **制約:** 住民税についても同様の控除(地方税)が存在するとされるが、
 * 一次情報で条文・算式を確実に確認できていないため、本ツールでは所得税・
 * 復興特別所得税分の控除のみを試算する(住民税分は対象外。今後の課題)。
 */
...
export function estimateDistributionAdjustedForeignTaxCredit(
  ...
  const notes: string[] = [
    ...
    "住民税についても同様の控除があるとされるが、一次情報で条文・算式を確認できていないため本ツールでは試算しない(所得税・復興特別所得税分のみ)。",
  ];
