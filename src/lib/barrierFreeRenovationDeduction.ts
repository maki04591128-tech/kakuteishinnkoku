 ...
 * 41条の19の3。国税庁タックスアンサーNo.1220)。
 *
 * 自己が所有する居住用家屋について、高齢者等が安全に生活できるようにするための
 * バリアフリー改修工事(通路等の拡幅・階段の勾配緩和・浴室や便所の改良・手すりの
 * 設置・段差の解消・出入口の戸の改良・床材の滑り止め化等)を行い、平成26年4月1日
 * から令和10年(2028年)12月31日までの間に自己の居住の用に供した場合に、住宅ローン
 * 控除(`mortgageDeduction.ts`)・住宅耐震改修特別控除(`earthquakeRenovationDeduction.ts`)・
 * 省エネ改修工事の住宅特定改修特別税額控除(`energySavingRenovationDeduction.ts`)と
 * 同様、借入金の有無を問わずその年分の所得税額から控除できる制度。単年で完結する
 * 控除で繰越制度は無い。
 *
 * 対象者は次のいずれかに該当する「特定個人」が改修工事を行うことが要件
 * (国税庁タックスアンサーNo.1220):
 *  (1) 50歳以上の方
 *  (2) 介護保険法に規定する要介護または要支援の認定を受けている方
 *  (3) 所得税法上の障害者である方
 *  (4) 高齢者等(65歳以上の方または上記(2)もしくは(3)に該当する方)である
 *      親族と同居を常況としている方
 *
 * 計算式(国税庁タックスアンサーNo.1220に基づく。令和4年1月1日以後に居住の用に
 * 供した場合):
 *   控除額 = A×10% + B×5%(A・Bそれぞれ100円未満切り捨て)
 *   A = バリアフリー改修工事の標準的な費用の額(補助金等控除後。控除対象限度額
 *       200万円を限度)
 *   B = 次の(1)(2)のいずれか低い金額(1,000万円からAを控除した金額を限度)
 *       (1) 標準的な費用の額のうち控除対象限度額を超える部分 + 併せて行う
 *           増築・改築その他の一定の工事に要した費用の額(補助金等控除後)
 *       (2) バリアフリー改修工事の標準的な費用の額(頭打ち前)
 *
 * **制約:**
 *  - 住民税に相当する控除制度は存在しない(所得税のみの制度。住宅耐震改修
 *    特別控除・省エネ改修工事の住宅特定改修特別税額控除と同様)。
 *  - 令和4年(2022年)1月1日より前に居住の用に供した場合は、控除対象限度額が
 *    工事費用に含まれる消費税率(8%・10%)により200万円/150万円になる場合が
 *    あるなど算式・要件が異なり、本ツールはこの古い制度のケースを判定する
 *    ための情報を持たないため対象外とする(今後の課題)。
 *  - この控除は省エネ改修(41条の19の3)・住宅耐震改修特別控除(41条の19の2)・
 *    多世帯同居改修・耐久性向上改修・子育て対応改修の各控除と併用した場合、
 *    B(増改築等工事費用分)の1,000万円の限度額を合算して判定する必要があるが、
 *    本ツールはバリアフリー改修工事単独での適用を前提とし、他の改修工事との
 *    併用時のB限度額の合算判定は行わない(今後の課題)。
 */

const STANDARD_COST_MINIMUM_JPY = new Decimal(500_000);
const CONTROL_LIMIT_JPY = new Decimal(2_000_000);
...
export type BarrierFreeRenovationFloorAreaCategory = "AT_LEAST_50" | "FROM_40_TO_50";

/**
 * バリアフリー改修工事を行う人が該当する必要がある「特定個人」の区分
 * (国税庁タックスアンサーNo.1220の(1)〜(4)。いずれか1つに該当すればよい)
 */
export type BarrierFreeRenovationQualifyingPersonCategory =
  | "AGE_50_OR_OLDER"
...
export interface BarrierFreeRenovationDeductionInput {
  /** 改修後の住宅に自己の居住の用に供した年(西暦) */
  residenceYear: number;
  /** 床面積区分(50平方メートル以上か、40平方メートル以上50平方メートル未満か) */
  floorAreaCategory: BarrierFreeRenovationFloorAreaCategory;
  /** この特別控除を受ける年分の合計所得金額 */
  totalIncomeJpy: Decimal.Value;
  /** バリアフリー改修工事を行う人が該当する特定個人の区分(いずれにも該当しない場合はNONE) */
  qualifyingPersonCategory: BarrierFreeRenovationQualifyingPersonCategory;
  /** バリアフリー改修工事に係る標準的な費用の額(補助金等の額を控除した後の金額) */
  standardCostJpy: Decimal.Value;
  /** バリアフリー改修工事と併せて行う増築・改築その他の一定の工事に要した費用の額(補助金等控除後) */
  otherRelatedWorkCostJpy?: Decimal.Value;
  /** バリアフリー改修工事の日から6か月以内に居住の用に供したか */
  workCompletedWithinSixMonths: boolean;
  /** 工事費用の2分の1以上の額が自己の居住用部分の工事費用か */
  atLeastHalfCostForOwnResidence: boolean;
  /** その年の前年以前3年内の各年分に、同一住宅についてバリアフリー改修工事に係るこの税額控除の適用を受けたか */
  usedSameCreditInPastThreeYears: boolean;
}

export interface BarrierFreeRenovationDeductionResult {
  eligible: boolean;
...
/**
 * バリアフリー改修工事をした場合の住宅特定改修特別税額控除額を試算する。
 * DBに依存しない純粋関数。
 */
export function estimateBarrierFreeRenovationDeduction(
  input: BarrierFreeRenovationDeductionInput,
): BarrierFreeRenovationDeductionResult {
  ...
  const otherRelatedWorkCostJpy = input.otherRelatedWorkCostJpy
    ...
    : new Decimal(0);
  requireNonNegative(standardCostJpy, "バリアフリー改修工事に係る標準的な費用の額");
  requireNonNegative(totalIncomeJpy, "合計所得金額");
  requireNonNegative(otherRelatedWorkCostJpy, "併せて行う増築・改築その他の一定の工事に要した費用の額");

  const notes: string[] = [
    "租税特別措置法41条の19の3・国税庁タックスアンサーNo.1220(令和4年1月1日以後に居住の用に供した場合の算式)に基づく概算値。住民税に相当する控除制度は存在しないため、所得税額からのみ控除する(住宅耐震改修特別控除・省エネ改修工事の住宅特定改修特別税額控除と同様)。",
    "省エネ改修工事・住宅耐震改修特別控除・多世帯同居改修・耐久性向上改修・子育て対応改修等の他の住宅特定改修特別税額控除と併用する場合、B(増改築等工事費用分)の1,000万円の限度額は各改修工事のAの合計額を控除した額が限度になるが、本ツールはバリアフリー改修工事単独での適用を前提とし、他の改修工事との併用時の限度額の合算判定は行わない(今後の課題)。",
  ];

  function ineligible(reason: string): BarrierFreeRenovationDeductionResult {
    return {
      eligible: false,
  ...
  if (input.residenceYear < EARLIEST_SUPPORTED_RESIDENCE_YEAR) {
    return ineligible(
      `令和4年(2022年)より前に居住の用に供した場合は控除対象限度額の判定(工事費用に含まれる消費税率による200万円/150万円との区分等)が異なり、本ツールでは試算対象外とする。`,
    );
  }
  ...
  if (input.qualifyingPersonCategory === "NONE") {
    return ineligible(
      "バリアフリー改修工事を行う人が、50歳以上・要介護または要支援の認定・所得税法上の障害者・高齢者等(65歳以上または要介護等の認定を受けた親族)と同居のいずれかに該当する「特定個人」であることが要件。",
    );
  }
  ...
  if (standardCostJpy.lessThanOrEqualTo(STANDARD_COST_MINIMUM_JPY)) {
    return ineligible(
      "バリアフリー改修工事に係る標準的な費用の額(補助金等控除後)が50万円を超えることが要件。",
    );
  }
  if (!input.workCompletedWithinSixMonths) {
    return ineligible("バリアフリー改修工事の日から6か月以内に居住の用に供していることが要件。");
  }
  ...
  if (input.usedSameCreditInPastThreeYears) {
    return ineligible(
      "その年の前年以前3年内の各年分において、同一の住宅についてバリアフリー改修工事に係るこの税額控除を適用した場合、重ねて適用することはできない。",
    );
  }
  ...
  if (standardCostJpy.greaterThan(CONTROL_LIMIT_JPY)) {
    notes.push(
      `バリアフリー改修工事に係る標準的な費用の額が控除対象限度額(${CONTROL_LIMIT_JPY.toString()}円)を超えるため、超過分は増築・改築その他の一定の工事に要した費用の額と合わせてB(5%控除)の対象とした。`,
    );
  }
...
export interface BarrierFreeRenovationDeductionRecordEntry {
  taxYear: number;
  /** その年分の控除額(所得税分。BarrierFreeRenovationDeductionResult.creditJpy) */
  creditJpy: Decimal;
}

/**
 * `/barrier-free-renovation-deduction`で登録済みの、指定した年分のバリアフリー改修
 * 住宅特定改修特別税額控除額をDBから読み出す。省エネ改修工事の住宅特定改修特別
 * 税額控除と同様、下書きCSV(`/api/export`)の税額控除欄・`/tax-estimate`の合計税額
 * 試算への自動反映に使う。未登録の年は null を返す。
 */
