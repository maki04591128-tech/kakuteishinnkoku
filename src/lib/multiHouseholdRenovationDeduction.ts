 ...
 * 41条の19の3。国税庁タックスアンサーNo.1224)。
 *
 * 自己が所有する居住用家屋について、複数世帯が同居しやすくするための多世帯同居
 * 改修工事(調理室・浴室・便所・玄関のいずれかを増設する工事)を行い、平成28年
 * (2016年)4月1日から令和10年(2028年)12月31日までの間に自己の居住の用に供した
 * 場合に、住宅ローン控除(`mortgageDeduction.ts`)・住宅耐震改修特別控除
 * (`earthquakeRenovationDeduction.ts`)・省エネ改修工事の住宅特定改修特別税額控除
 * (`energySavingRenovationDeduction.ts`)・バリアフリー改修工事の住宅特定改修特別
 * 税額控除(`barrierFreeRenovationDeduction.ts`)と同様、借入金の有無を問わずその年分
 * の所得税額から控除できる制度。単年で完結する控除で繰越制度は無い。
 *
 * 対象となる多世帯同居改修工事は、調理室・浴室・便所・玄関のいずれか2以上について、
 * 改修後に自己の居住の用に供する部分でそれぞれ複数になる工事に限られる
 * (国税庁タックスアンサーNo.1224)。
 *
 * 計算式(国税庁タックスアンサーNo.1224に基づく):
 *   控除額 = A×10% + B×5%(A・Bそれぞれ100円未満切り捨て)
 *   A = 多世帯同居改修工事の標準的な費用の額(補助金等控除後。控除対象限度額
 *       250万円を限度)
 *   B = 次の(1)(2)のいずれか低い金額(1,000万円からAを控除した金額を限度)
 *       (1) 標準的な費用の額のうち控除対象限度額を超える部分 + 併せて行う
 *           増築・改築その他の一定の工事に要した費用の額(補助金等控除後)
 *       (2) 多世帯同居改修工事の標準的な費用の額(頭打ち前)
 *
 * **制約:**
 *  - 住民税に相当する控除制度は存在しない(所得税のみの制度。他の住宅特定改修
 *    特別税額控除と同様)。
 *  - 令和4年(2022年)1月1日より前に居住の用に供した場合は、バリアフリー改修工事・
 *    省エネ改修工事と同様に控除対象限度額が工事費用に含まれる消費税率(8%・10%)
 *    により異なる場合があるなど算式・要件が異なり、本ツールはこの古い制度のケースを
 *    判定するための情報を持たないため対象外とする(今後の課題)。
 *  - この控除はバリアフリー改修・省エネ改修(41条の19の3)・住宅耐震改修特別控除
 *    (41条の19の2)・耐久性向上改修・子育て対応改修の各控除と選択適用(いずれか
 *    1つのみ)となるほか、併用時のB(増改築等工事費用分)の1,000万円の限度額の
 *    合算判定が必要だが、本ツールは多世帯同居改修工事単独での適用を前提とし、
...
export type MultiHouseholdRenovationFloorAreaCategory = "AT_LEAST_50" | "FROM_40_TO_50";
...
export interface MultiHouseholdRenovationDeductionInput {
  /** 改修後の住宅に自己の居住の用に供した年(西暦) */
  residenceYear: number;
  /** 床面積区分(50平方メートル以上か、40平方メートル以上50平方メートル未満か) */
  floorAreaCategory: MultiHouseholdRenovationFloorAreaCategory;
  /** この特別控除を受ける年分の合計所得金額 */
  totalIncomeJpy: Decimal.Value;
  /** 改修工事により調理室が複数になったか */
  duplicatedKitchen: boolean;
  /** 改修工事により浴室が複数になったか */
  duplicatedBathroom: boolean;
  /** 改修工事により便所が複数になったか */
  duplicatedToilet: boolean;
  /** 改修工事により玄関が複数になったか */
  duplicatedEntrance: boolean;
  /** 多世帯同居改修工事に係る標準的な費用の額(補助金等の額を控除した後の金額) */
  standardCostJpy: Decimal.Value;
  /** 多世帯同居改修工事と併せて行う増築・改築その他の一定の工事に要した費用の額(補助金等控除後) */
  otherRelatedWorkCostJpy?: Decimal.Value;
  /** 多世帯同居改修工事の日から6か月以内に居住の用に供したか */
  workCompletedWithinSixMonths: boolean;
  /** 工事費用の2分の1以上の額が自己の居住用部分の工事費用か */
  atLeastHalfCostForOwnResidence: boolean;
  /** その年の前年以前3年内の各年分に、同一住宅について多世帯同居改修工事に係るこの税額控除の適用を受けたか */
  usedSameCreditInPastThreeYears: boolean;
}

export interface MultiHouseholdRenovationDeductionResult {
  eligible: boolean;
...
/**
 * 多世帯同居改修工事をした場合の住宅特定改修特別税額控除額を試算する。
 * DBに依存しない純粋関数。
 */
export function estimateMultiHouseholdRenovationDeduction(
  input: MultiHouseholdRenovationDeductionInput,
): MultiHouseholdRenovationDeductionResult {
  ...
  const otherRelatedWorkCostJpy = input.otherRelatedWorkCostJpy
    ...
    : new Decimal(0);
  requireNonNegative(standardCostJpy, "多世帯同居改修工事に係る標準的な費用の額");
  requireNonNegative(totalIncomeJpy, "合計所得金額");
  requireNonNegative(otherRelatedWorkCostJpy, "併せて行う増築・改築その他の一定の工事に要した費用の額");

  const notes: string[] = [
    "租税特別措置法41条の19の3・国税庁タックスアンサーNo.1224に基づく概算値。住民税に相当する控除制度は存在しないため、所得税額からのみ控除する(他の住宅特定改修特別税額控除と同様)。",
    "バリアフリー改修・省エネ改修・住宅耐震改修特別控除・耐久性向上改修・子育て対応改修の各控除といずれか1つの選択適用となるほか、併用時のB(増改築等工事費用分)の1,000万円の限度額は各改修工事のAの合計額を控除した額が限度になるが、本ツールは多世帯同居改修工事単独での適用を前提とし、他の改修工事との選択適用・併用時の限度額の合算判定は行わない(今後の課題)。",
  ];

  function ineligible(reason: string): MultiHouseholdRenovationDeductionResult {
    return {
      eligible: false,
  ...
  const duplicatedRoomCount = [
    input.duplicatedKitchen,
    input.duplicatedBathroom,
    input.duplicatedToilet,
    input.duplicatedEntrance,
  ].filter(Boolean).length;
  if (duplicatedRoomCount < 2) {
    return ineligible(
      "自己の居住の用に供する部分において、調理室・浴室・便所・玄関のうちいずれか2以上の室がそれぞれ複数になる工事であることが要件。",
    );
  }
  ...
  if (standardCostJpy.lessThanOrEqualTo(STANDARD_COST_MINIMUM_JPY)) {
    return ineligible(
      "多世帯同居改修工事に係る標準的な費用の額(補助金等控除後)が50万円を超えることが要件。",
    );
  }
  if (!input.workCompletedWithinSixMonths) {
    return ineligible("多世帯同居改修工事の日から6か月以内に居住の用に供していることが要件。");
  }
  ...
  if (input.usedSameCreditInPastThreeYears) {
    return ineligible(
      "その年の前年以前3年内の各年分において、同一の住宅について多世帯同居改修工事に係るこの税額控除を適用した場合、重ねて適用することはできない。",
    );
  }
  ...
  if (standardCostJpy.greaterThan(CONTROL_LIMIT_JPY)) {
    notes.push(
      `多世帯同居改修工事に係る標準的な費用の額が控除対象限度額(${CONTROL_LIMIT_JPY.toString()}円)を超えるため、超過分は増築・改築その他の一定の工事に要した費用の額と合わせてB(5%控除)の対象とした。`,
    );
  }
...
export interface MultiHouseholdRenovationDeductionRecordEntry {
  taxYear: number;
  /** その年分の控除額(所得税分。MultiHouseholdRenovationDeductionResult.creditJpy) */
  creditJpy: Decimal;
}

/**
 * `/multi-household-renovation-deduction`で登録済みの、指定した年分の多世帯同居改修
 * 住宅特定改修特別税額控除額をDBから読み出す。バリアフリー改修工事・省エネ改修工事の
 * 住宅特定改修特別税額控除と同様、下書きCSV(`/api/export`)の税額控除欄・
 * `/tax-estimate`の合計税額試算への自動反映に使う。未登録の年は null を返す。
 */
export async function getMultiHouseholdRenovationDeductionRecord(
  year: number,
): Promise<MultiHouseholdRenovationDeductionRecordEntry | null> {
  const taxYear = await prisma.taxYear.findUnique({ where: { year } });
  if (!taxYear) return null;

  const record = await prisma.multiHouseholdRenovationDeductionRecord.findUnique({
