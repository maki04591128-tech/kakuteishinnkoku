 ...
 * この試算では控除額を同額として扱う。
 *
 * この関数が返す控除額はその年の総所得金額等による上限をかける前の金額であり、
 * 総所得金額等を超えて控除しきれなかった場合の翌年以後3年間の繰越控除
 * (雑損失の繰越控除)は src/lib/casualtyLossCarryforward.ts で別途計算する。
 */
...
export function estimateCasualtyLossDeduction(
  ...
  const notes: string[] = [
    ...
    "この控除額は総所得金額等による上限をかける前の金額。総所得金額等を超えて控除しきれなかった場合に翌年以後3年間繰り越せる「雑損失の繰越控除」は、この試算画面上部で発生年ごとの繰越残高を加味した結果として別途表示する。",
  ];
