 ...
 * 人的控除額の差になる。
 *
 * **令和8年分(2026年分)以後の扱い(未検証・今後の課題):** 当初`src/lib/basicDeduction.ts`は
 * 所得税の基礎控除の132万円超655万円以下の時限的な上乗せ(88万円/68万円/63万円)が
 * 令和7年分・8年分限定で令和9年分以後は58万円に統一される制度設計を前提にしており、
 * その場合に336万円以下の層を5万円に据え置く経過措置が令和9年分以後も続くかどうかを
 * 洲本市ページを含む一次情報で確認できていないことを課題としていた。
 *
 * その後、令和8年度税制改正(令和7年12月26日閣議決定)により、上記の制度設計自体が
 * 施行前に置き換わった。物価連動の基礎控除引上げが恒久制度として新設され、`src/lib/
 * basicDeduction.ts`が実装する通り、令和8年分・9年分の所得税の基礎控除額は132万円以下・
 * 132万円超336万円以下・336万円超489万円以下がいずれも104万円、489万円超655万円以下は
 * 67万円、655万円超2,350万円以下は62万円になり(令和7年分の95万円/88万円/68万円/63万円/
 * 58万円から変わり)、令和10年分以後は132万円以下99万円・132万円超2,350万円以下62万円に
 * 統一される。これにより、令和8年分以後の基礎控除の人的控除額の差(所得税の基礎控除額-
 * 住民税の基礎控除額43万円)は、令和7年分向けに整理した`BASIC_DEDUCTION_DIFF_AFTER_REFORM`
 * (95万円/88万円/68万円/63万円/58万円を前提にした表)とは異なる値になるはずだが、
 * 令和8年度税制改正を踏まえた住民税側の人的控除額の差の一覧を示す自治体公式ページを
 * 本稿執筆時点でまだ確認できていない(大津市・大阪市等の令和8年度改正の案内ページにも
 * 調整控除の差額表自体の更新は見当たらなかった)。誤って過大な調整控除額を案内する
 * リスクを避けるため、本モジュールは令和8年分以後も暫定的に令和7年分向けの表
 * (`BASIC_DEDUCTION_DIFF_AFTER_REFORM`)をそのまま適用し、`estimateResidentTaxAdjustmentDeduction`
 * の返り値`notes`に令和8年分以後は未検証である旨を明記する(従来は令和9年分以後のみを
 * 未検証としていたが、令和8年分の時点で既に前提が変わっているため対象年分を1年前倒しした)。
 *
...
/**
 * この年分以後は、令和8年度税制改正による所得税の基礎控除額のさらなる引上げを踏まえた
 * 基礎控除の人的控除額の差の扱いが一次情報で確認できておらず、令和7年分向けの表を暫定適用する
 */
...
export function estimateResidentTaxAdjustmentDeduction(
  ...
  if ((input.personalDeductionDifference.year ?? 0) >= BASIC_DEDUCTION_DIFF_UNVERIFIED_YEAR) {
    notes.push(
      "令和8年分(2026年分)以後は、令和8年度税制改正による所得税の基礎控除額のさらなる引上げ(令和7年分の95万円/88万円/68万円/63万円/58万円から令和8・9年分は104万円/67万円/62万円等に変更)が住民税の調整控除の人的控除額の差にどう反映されるか一次情報で確認できていないため、令和7年分向けの表を暫定適用した参考値である。",
    );
  }
