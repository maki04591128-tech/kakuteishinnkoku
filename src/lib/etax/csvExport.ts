...
const INCOME_DEDUCTION_FILING_LOCATION: Record<IncomeDeductionType, string> = {
  ...
  HOME_SALE_LOSS:
    "申告書第一表 所得から差し引かれる金額(雑損控除等の欄。譲渡所得の損失として総所得金額等と損益通算) / 第四表(損失申告用) / 特定居住用財産の譲渡損失の金額の明細書",
  HOME_REPLACEMENT_LOSS:
    "申告書第一表 所得から差し引かれる金額(雑損控除等の欄。譲渡所得の損失として総所得金額等と損益通算) / 第四表(損失申告用) / 居住用財産の買換え等に係る譲渡損失の金額の明細書",
};
...
export function buildTaxFilingDraftCsv(
  ...
  if (
    ...
    if (summary.energySavingRenovationDeduction) {
      lines.push(
        toCsvLine([
          "省エネ改修工事の住宅特定改修特別税額控除額: 所得税からの控除額(住民税に相当する控除は無し)",
          formatYen(summary.energySavingRenovationDeduction.creditJpy),
          "申告書第一表 税額控除(住宅特定改修特別税額控除) / 住宅耐震改修特別控除額・住宅特定改修特別税額控除額の計算明細書",
        ]),
      );
    }
    if (summary.barrierFreeRenovationDeduction) {
      lines.push(
        toCsvLine([
          "バリアフリー改修工事の住宅特定改修特別税額控除額: 所得税からの控除額(住民税に相当する控除は無し)",
          formatYen(summary.barrierFreeRenovationDeduction.creditJpy),
          "申告書第一表 税額控除(住宅特定改修特別税額控除) / 住宅耐震改修特別控除額・住宅特定改修特別税額控除額の計算明細書",
        ]),
      );
    }
    if (summary.multiHouseholdRenovationDeduction) {
      lines.push(
        toCsvLine([
    ...
    if (summary.childRearingRenovationDeduction) {
      lines.push(
        toCsvLine([
